...
export class CreateTransactionDto {
    ...
    @IsDecimal({ decimal_digits: '0,8' }, { message: 'Quantity must be a decimal with up to 8 decimal places' })
    quantity: number;

    @ApiProperty({ description: 'Unit price of the asset' })
    @IsNotEmpty({ message: 'Unit price is required' })
    @IsDecimal({ decimal_digits: '0,8' }, { message: 'Unit price must be a decimal with up to 8 decimal places' })
    unitPrice: number;

    @ApiProperty({ description: 'Total value of the transaction' })
    @IsNotEmpty({ message: 'Total value is required' })
    @IsDecimal({ decimal_digits: '0,8' }, { message: 'Total value must be a decimal with up to 8 decimal places' })
    totalValue: number;
    ...
    @IsOptional()
    @IsDecimal({ decimal_digits: '0,8' }, { message: 'Fee must be a decimal with up to 8 decimal places' })
    fee?: number;
