...
export class AssetQuotesController {
    ...
    @ApiResponse({ status: 201, description: 'Quote successfully created', type: AssetQuoteResponseDto })
    @ApiResponse({ status: 400, description: 'Invalid data' })
    @ApiResponse({ status: 404, description: 'Asset not found' })
    async create(@Body() createAssetQuoteDto: CreateAssetQuoteDto): Promise<AssetQuoteResponseDto> {
    ...
    @ApiQuery({ name: 'assetId', required: true, description: 'Asset ID', type: Number })
    @ApiResponse({ status: 200, description: 'Latest quote found', type: AssetQuoteResponseDto })
    @ApiResponse({ status: 404, description: 'Quote not found for the asset provided.' })
    async findLatest(@Query('assetId') assetId: string): Promise<AssetQuoteResponseDto> {
        const quote = await this.assetQuotesService.findLatestByAssetId(Number(assetId));
        return new AssetQuoteResponseDto(quote);
    }

    @Get('latest/all')
    @ApiOperation({ summary: 'Get the latest quote for all assets' })
    @ApiResponse({ status: 200, description: 'Latest quotes for all assets', type: [AssetQuoteResponseDto] })
    ...
    @ApiResponse({ status: 200, description: 'Paginated list of latest quotes for all assets', type: PaginatedAssetQuoteResponseDto })
    async findLatestForAllAssetsWithPagination(
        ...
        const quotes = await this.assetQuotesService.findLatestForAllAssetsWithPagination(takeNumber, pageNumber);
    ...
    @ApiResponse({ status: 200, description: 'Quote history', type: [AssetQuoteResponseDto] })
    @ApiResponse({ status: 404, description: 'No quotes found for the asset.' })
    async findAllByAssetId(@Param('assetId') assetId: string): Promise<AssetQuoteResponseDto[]> {
        const quotes = await this.assetQuotesService.findAllByAssetId(Number(assetId));
        return quotes.map(q => new AssetQuoteResponseDto(q));
    }
    ...
    @ApiResponse({ status: 200, description: 'Quote found', type: AssetQuoteResponseDto })
    ...
    @ApiParam({ name: 'id', description: 'Quote ID' })
    @ApiResponse({ status: 200, description: 'Quote updated', type: AssetQuoteResponseDto })
    ...
    async update(
        ...
        const updated = await this.assetQuotesService.update(Number(id), updateData);
    ...
    @ApiParam({ name: "id", description: "Quote ID" })
    @ApiResponse({ status: 200, description: "Quote has been marked as successfully removed" })
    @ApiResponse({ status: 404, description: "Quote not found." })
